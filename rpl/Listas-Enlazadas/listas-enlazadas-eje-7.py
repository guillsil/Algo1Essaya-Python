"""Implementar el método invertir de ListaEnlazada que la modifica invirtiendo su orden
(es decir, el primer elemento queda como último y viceversa). El método no debe recorrer la lista
 N o más veces (siendo N la cantidad de elementos de la lista).
...
class ListaEnlazada:
    ...
    def invertir(self):
        ...
        while act:
            sig = act.prox
            act.prox = ant  # el cambio de flecha en sí!
            ant = act
            act = sig
    ...
    def filter(self, f):
        """Devuelve una nueva lista enlazada con los elementos para los cuales la
        aplicación de f devuelve True."""
        #No se puede usar append
        nueva = ListaEnlazada()
        act_self = self.prim
        act_nueva = None
        while act_self:
            # Muy similar al map, solo que acá vamos a insertar "condicionalmente"
            if f(act_self.dato):
                if act_nueva:
